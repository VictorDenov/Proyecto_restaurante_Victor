...
$sql=$conn->prepare("SELECT * FROM factura_cliente,cliente WHERE factura_cliente.cus_id=? AND factura_cliente.ci_fecha=? AND factura_cliente.ci_order_no=? AND factura_cliente.cus_id=customer.cus_id");
...
$sql1=$conn->prepare("SELECT * FROM detalles_carrito_producto,detalles_inventario WHERE detalles_carrito_producto.cus_id=? AND detalles_carrito_producto.pcd_order_no=? AND detalles_carrito_producto.pcd_date=? AND detalles_carrito_producto.pd_id=detalles_inventario.pd_id");
...
while($row1=$result1->fetch_assoc())
{
    ...
    $pdf->Cell(85 ,10,$row1['pcd_nombre'],1,0);
    $pdf->Cell(25 ,10,$row1['pcd_precio'],1,0,'R');
    $pdf->Cell(20 ,10,$row1['sd_descuento'],1,0,'R');
    $pdf->Cell(15 ,10,$row1['pcd_cantidad'],1,0,'C');
    $pdf->SetFont('Arial','B',12);
    $pdf->Cell(29 ,10,$row1['pcd_monto_total'],1,1,'R');//end of line
}
    //summary
//    $pdf->Cell(125 ,10,'',0,0);
    $pdf->SetFont('Arial','B',12);
