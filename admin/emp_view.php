...
<?php
  ...
  $sql=$conn->prepare("SELECT * FROM SUCURSAL,eMPLEADO where sucursal.b_id=empleado.b_id");
  $sql->execute();
...
?>
  <tr>
    <td><?php echo $sno++;?></td>
    <td><?php echo $row["b_nombre"];?></td>
    <td> <?php echo $row["e_nombre"]; ?> </td>  
    <td><?php echo $row["e_contacto"];?></td>    
    <td><?php echo $row["e_direccion"];?></td>
    <td><?php echo $row["e_email"];?></td>
    <td><?php echo $row["e_username"];?></td>
    <td><?php echo $row["e_password"];?></td>
    <td><?php echo $row["e_fecha"];?></td>
