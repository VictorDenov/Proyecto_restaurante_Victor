<?php
    ...
    $sql=$conn->prepare("SELECT * FROM detalles_carrito_producto WHERE cus_id=? AND pcd_date=?");
    ...
    if($result->num_rows > 0 )
    {
        ...
        while($row=$result->fetch_assoc())
        {
            ...
            $sql2=$conn->prepare("SELECT * FROM detalles_carrito_producto WHERE pcd_id=?");
            ...
            $sql3=$conn->prepare("SELECT * FROM detalles_inventario WHERE pd_id=?");
            ...
            $sql5=$conn->prepare("UPDATE detalles_inventario SET sd_cantidad=?,sd_estado=? WHERE sd_id=? ");
